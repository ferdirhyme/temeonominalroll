...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const getUserProfile = async (supabaseUser: any): Promise<User | null> => {
        try {
            ...
            if (userError || !userAccount) {
                console.warn(`Could not fetch user profile: ${userError?.message || 'User not found.'}`);
                return null;
            }
            ...
            if (staffError) console.warn(`Could not fetch ippd profile for ${userAccount.staffId}:`, staffError);

            if (staffProfile?.is_archived) {
                console.warn('This account has been archived and cannot be accessed.');
                return null;
            }
    ...
    const signup = async ({ staffId, email, password, emiscode }: { staffId: string; email: string; password: string; emiscode: number }) => {
        ...
        const role = staffType?.toLowerCase() === 'headteacher' ? UserRole.Admin : UserRole.Teacher;
